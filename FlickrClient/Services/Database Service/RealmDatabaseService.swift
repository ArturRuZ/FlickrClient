...
class RealmDatabaseService: IDatabaseService {
    ...
    func updateObjectsStateInBase(fromData: PhotosModel)  {
        ...
        prepareDateForUpdate.id = fromData.id
        prepareDateForUpdate.title = fromData.title
        prepareDateForUpdate.url = fromData.url
        if fromData.isFavorite {prepareDateForUpdate.isFavorite.value = true} else {prepareDateForUpdate.isFavorite.value = false}
